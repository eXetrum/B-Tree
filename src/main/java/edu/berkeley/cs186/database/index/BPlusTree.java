...
public class BPlusTree {
  ...
  {
    return BPlusNode.getBPlusNode(this, rootPageNum);
   ...
   */
    ...
    public BPlusIterator(LeafNode leaf, DataType key, boolean scan) {
      currLeaf = leaf;
      lookupKey = key;
      currLeafIter = scan ? leaf.scanFrom(key) : leaf.scanForKey(key);
      isScan = scan;
    }

    public boolean hasNext() {
      if(currLeafIter.hasNext()) return true;

      if(currLeaf.getNextLeaf() == -1) return false;
    ...
    public RecordID next() {
      if(currLeafIter.hasNext()) return currLeafIter.next();
